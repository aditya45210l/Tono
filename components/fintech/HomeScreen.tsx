...
import { ArrowDownLeft, ArrowUpRight, CreditCard, QrCode, Bell, Send, MoreHorizontal } from "lucide-react";
...
export default function HomeScreen({ onNavigate }: HomeScreenProps) {
  ...
  return (
    <div
      ref={containerRef}
      className="relative h-full bg-black text-white pb-24 overflow-y-auto">
      ...
      <div className="relative z-10 px-5 pt-6">
        ...
        <motion.div
          ...
          className="flex justify-between gap-2 mb-8 px-2">

          {[
            { icon: Send, label: "Send", color: "bg-[#9EFF36]/10", screen: "payment" as Screen },
            { icon: QrCode, label: "Manage QR", color: "bg-[#9EFF36]/10", screen: "manage-qr" as Screen },
            { icon: MoreHorizontal, label: "More", color: "bg-[#9EFF36]/10", screen: "home" as Screen },
            { icon: QrCode, label: "Scan", color: "bg-[#9EFF36]/10", screen: "scan" as Screen }
          ].map((action, idx) => (
            <motion.button
              key={action.label}
              whileTap={{ scale: 0.95 }}
            ...
            >
              <div className={`w-[68px] h-[68px] rounded-[24px] ${action.color} glass flex items-center justify-center border border-[#9EFF36]/20 hover:border-[#9EFF36]/50 transition-colors`}>
                <action.icon className="w-7 h-7 text-[#9EFF36]" strokeWidth={1.5} />
              </div>
        ...
        {/* Transactions List with Scroll Animation */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
